	...
	var captchaSuccess = function(){
		...
		mkdirp( path, function (err) {
			if (err) {
				...
				// insert if scene does not exist
				if(!sceneId){
					db.serialize(function() {

						var stmt = db.prepare("INSERT INTO scene ( id, description, name, location, timestamp, removehash, images, user_id ) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?)");
						stmt.run([ req.form.data.description, req.form.data.scenename, location, timestamp, removeHash, images.length, userId ], function(error){
							if(error) {
								s['error-codes'] = error;
								return res.status(400).send(s);
								//console.log(error);
					...
					// update if scene exists
					db.serialize(function() {

						var stmt = db.prepare("UPDATE scene SET location = ?,  timestamp = ?, removehash = ?, images = ? WHERE id = ?");
						stmt.run([ location, timestamp, removeHash, images.length, sceneId ], function(error){
							if(error) {
								s['error-codes'] = error;
								return res.status(400).send(s);
								//console.log(error);
