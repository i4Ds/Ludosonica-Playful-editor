var Viewport = function ( editor ) {
	...
	signals.rendererChanged.add( function ( type ) {
		...
		if ( container.effect ) container.effect.setSize( container.dom.offsetWidth, container.dom.offsetHeight );
	...
	if ( editor.config.getKey( 'effect' ) !== undefined ) {
		...
		container.effect = new THREE[ editor.config.getKey( 'effect' ) ]( renderer );
	...
	signals.effectChanged.add( function( type ) {
		...
		switch ( type ) {
			case 'StereoEffect':
				container.effect = new THREE.StereoEffect( renderer );
				container.effect.separation = 0.2; //wrong way round?
				break;
			case undefined:
	...
	function render() {
		...
		renderer.clear();
		(container.effect && container.play ? container.effect : renderer).render( scene, editor._activeCamera );
