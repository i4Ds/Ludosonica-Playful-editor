	...
	return {
		...
		set: function ( data, callback ) {
			...
			request.onsuccess = function ( event ) {
				...
				Logger.info('[' + /\d\d\:\d\d\:\d\d/.exec( new Date() )[ 0 ] + ']', 'Saved state to IndexedDB. ' + ( performance.now() - start ).toFixed( 2 ) + 'ms', "logged by Storage.js row 102");
			};
				...
				cwd.getFile(file.name, {create: true}, function( fileEntry ) {
					...
					fileEntry.createWriter( function( fileWriter ) {
						...
						fileWriter.onwriteend = function() {
							...
							//Logger.info('[' + /\d\d\:\d\d\:\d\d/.exec( new Date() )[ 0 ] + ']', 'Saved state to IndexedDB. ' + ( performance.now() - start ).toFixed( 2 ) + 'ms', "logged by Storage.js row 124");
						};
