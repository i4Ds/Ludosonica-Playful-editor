Sidebars.Scene = function ( editor ) {
	...
	signals.sceneGraphChanged.add( function () {
		...
		( function addObjects( objects, pad ) {
			...
			for ( var i = 0, l = objects.length; i < l; i ++ ) {
				...
				if ( editor.omittedObjects.indexOf( object.name ) === -1 ) {
					...
					html += ' <span class="icn icon-template" title="Duplicate object as template"></span>';
		...
		for ( var x = 0; x < options.length; x++ ) {
			...
			var temlateBtn = child.find('.icon-template');
			...
			if ( temlateBtn.length > 0 ) $( temlateBtn[0] ).click( function ( el ) {
				...
				editor.templateManager.addTemplate( clone );
